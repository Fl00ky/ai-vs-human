...
function QuizPlay({ onFinish }: { onFinish: (score: number) => void }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setTimeLeft(left);
      if (left <= 0) { clearInterval(interval); answer(null); }
    }, 50);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between text-xs">
        <span className="text-side/70 uppercase tracking-widest">
          {t.quiz.round} {round + 1} {t.quiz.of} {ROUNDS}
        </span>
        <span className="text-side font-display text-lg tabular-nums">{score}</span>
      </div>

      <div className="h-1 bg-side/20 overflow-hidden">
        <motion.div className="h-full bg-side" style={{ width: `${(timeLeft / ROUND_MS) * 100}%` }} transition={{ ease: "linear" }} />
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={round} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
          ...
          <span className="absolute top-2 left-3 text-[10px] text-side/40">
            &gt; {t.quiz.fragment}_{round + 1}.txt
          </span>
          <p className="text-base sm:text-lg leading-relaxed text-fg/90 font-mono">&ldquo;{current.text}&rdquo;</p>
        </motion.div>
      </AnimatePresence>

      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        <ChoiceButton color="ai" disabled={!!reveal} onClick={() => answer("ai")}
          highlight={reveal && current.source === "ai" ? "reveal" : null}
          chosen={reveal === "correct" && current.source === "ai"}>
          {t.common.ai}
        </ChoiceButton>
        <ChoiceButton color="human" disabled={!!reveal} onClick={() => answer("human")}
...
function ChoiceButton({ color, children, disabled, onClick, highlight, chosen }: {
  color: "ai" | "human"; children: React.ReactNode;
  disabled: boolean; onClick: () => void;
  highlight: "reveal" | null; chosen: boolean;
}) {
  ...
  return (
    <motion.button onClick={onClick} disabled={disabled}
      ...
      style={{ color: c, borderColor: chosen ? c : `${c}66`, background: chosen ? `${c}22` : "rgba(0,0,0,0.4)" }}>
      {children}
    </motion.button>
  );
}
