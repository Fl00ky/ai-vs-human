    ...
    <div className="space-y-8">
      ...
      <MotionGrid className="grid sm:grid-cols-2 gap-4">
        {quests.map((q) => {
          ...
          const loc = localizeQuest(lang, q);
          return (
            <MotionGridItem key={q.id}>
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <span className="font-display text-lg text-side uppercase tracking-wider">{loc.title}</span>
                ...
                <p className="text-sm text-fg/60">{loc.description}</p>
                {q.side && (
