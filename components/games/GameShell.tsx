...
interface GameShellProps {
  ...
  description: string;
  game: GameKind;
  /** Render the game itself, given a callback to call when the game ends with the final score. */
  render: (onFinish: (score: number, extras?: GameExtras) => void) => ReactNode;
  /** Optional HUD content shown above the play area during play (timer, round, etc.). */
  renderHud?: () => ReactNode;
}

interface GameExtras {
  /** For reaction: best ms this play (lower=better). For pattern: highest round reached. */
  detail?: number;
...
export function GameShell({
  ...
  const finish = async (final: number, extras: GameExtras = {}) => {
    ...
    if (final > 0) {
      confetti({
        particleCount: Math.min(200, Math.floor(final / 10)),
        spread: 70,
        origin: { y: 0.6 },
        colors: ["#00ff41", "#ff003c", "#00d4ff"],
      });
    }
    // Save automatically after a short pause
    setTimeout(async () => {
      setPhase("saving");
      try {
        ...
        // Show each unlocked achievement as its own toast, staggered
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        {phase === "playing" && renderHud && (
          <div className="text-right">{renderHud()}</div>
        )}
      </div>
      ...
      <div className="terminal-box p-6 min-h-[420px] flex items-center justify-center relative">
        <AnimatePresence mode="wait">
          {phase === "idle" && (
            ...
            >
              <div className="text-[10px] uppercase tracking-[0.3em] text-side/60 mb-4">
                [ ready ]
              </div>
              <button onClick={start} className="btn-matrix text-base px-8 py-3">
                <Play size={16} className="mr-2" /> Start mission
              </button>
            </motion.div>
          )}
          ...
          {phase === "boot" && (
            ...
            >
              <BootSequence
                lines={[
                  { text: `> loading mission_${game}...`, speed: 26 },
          ...
          {phase === "ready" && (
            ...
            >
              <motion.button
                ...
                className="btn-matrix text-base px-8 py-3"
                animate={{
                  boxShadow: [
                    "0 0 0 0 var(--side-color)",
                    "0 0 20px 2px var(--side-color)",
                    "0 0 0 0 var(--side-color)",
                  ],
                }}
                transition={{ duration: 1.8, repeat: Infinity }}
              >
                <Play size={16} className="mr-2" /> Begin
              </motion.button>
            </motion.div>
          )}
          ...
          {(phase === "finished" || phase === "saving" || phase === "saved") && (
            <motion.div
              ...
              className="text-center"
            >
              <div className="text-[10px] uppercase tracking-[0.3em] text-side/60 mb-2">
                [ mission complete ]
              </div>
              <div className="font-display text-6xl sm:text-7xl text-side tabular-nums mb-2 drop-shadow-[0_0_20px_var(--side-color)]">
                {formatScore(score)}
              </div>
              <div className="text-xs text-fg/50 mb-6">
                points contributed to your side
              </div>
              {phase === "saving" && (
                <div className="text-xs text-side animate-pulse">
                  Uploading to the grid...
                </div>
              )}
              {phase === "saved" && (
                <div className="text-xs text-matrix-green">
                  ✓ Recorded on the war board
                </div>
              )}
              {error && (
                <div className="text-xs text-ai-red mb-3">! {error}</div>
              )}
